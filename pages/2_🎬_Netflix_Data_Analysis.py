...
movies_df['country'] = movies_df['country'].apply(lambda x: ', '.join(x) if isinstance(x, list) else x)
unique_countries = pd.Series(movies_df['country'].str.split(', ').sum()).unique()
...
movies_df['country'].replace('Unknown', 'Other', inplace=True)

# Filter data by the given year
filtered_df = movies_df.loc[movies_df['release_year'] == year]
...
movies_df['duration'] = movies_df['duration'].apply(lambda x: int(x.split(' ')[0]) if 'min' in x else None)
