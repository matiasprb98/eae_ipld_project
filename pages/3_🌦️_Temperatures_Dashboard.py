...
# ----- Page configs -----
st.set_page_config(
    page_title="Matias Proano Portfolio",
    page_icon="📊",
)
...
if unique_countries_list is not None and len(selected_cities) > 0:
    ...
    for city in selected_cities:
        ...
        plt.title(f'Temperature Trends in Selected Cities ({start_date} to {end_date})')   # TODO
    ...
    for city in selected_cities:
        city_df =  temps_df[temps_df['City'] == city]           # TODO
        city_df_period = city_df[(city_df['Date'] >= start_date) & (city_df['Date'] <= end_date)]     # TODO
        plt.hist(city_df_period['AvgTemperatureCelsius'], bins=20, alpha=0.5, label=city)                # TODO

plt.title(f'Temperature Distributions in Selected Cities ({start_date} to {end_date})')   # TODO
